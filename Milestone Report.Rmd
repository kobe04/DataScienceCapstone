...
dFM <- dfm(sampleCorpus, remove = c(stopwords("english"), profanityList, "â"),
...
```{r bigram}
# Create plot of top-20 bigrams
tokenized <- tokens_remove(tokens(sampleCorpus, what = "word", remove_numbers = TRUE,
                                  remove_punct = TRUE, remove_separators = TRUE),
                           stopwords("english"))
tokenBi <- tokens_ngrams(tokenized, n = 2)
dFMBi <- dfm(tokenBi)
features_dFMBi <- textstat_frequency(dFMBi, n = 20)
...
```{r trigram}
# Create plot of top-20 trigram
tokenTri <- tokens_ngrams(tokenized, n = 3)
dFMTri <- dfm(tokenTri)
features_dFMTri <- textstat_frequency(dFMTri, n = 20)
...
The plots give an impression of the data. In general, they show that there is some further work to be done with cleaning the data. In the first plot, the 'word' "s" appears about 17,000 times in the sample.
