...
To assure that this project is reproducible, the seed is set. Then, the samples are taken and combined into one new file. It is decided that about 1% of the data in the files is included in the sample. These are the first three entries in the sample.
...
# Create the new file
sampleData <- c(sample(blogData, round(length(blogData) * 0.01)),
                sample(newsData, round(length(newsData) * 0.01)),
                sample(twitterData, round(length(twitterData) * 0.01)))
head(sampleData, 3)
...
To provide some exploratory statistics of the sample data, a Document-Term Matrix is created.
```{r DTM}
